...
mathematical set-builder notation. We present below the design currently
implemented at Jane Street.
...
# (* Flatten a nested array *)
  let sentences =
    ...
    |]
  in
  ([| word for sentence in sentences for word in sentence |] : _ iarray);;
- : string iarray =
...
### Array comprehensions and type-based disambiguation

We propose to generalize the mechanism of type-based disambiguation of array
literals to array comprehensions, in order to be able to construct immutable
arrays, float arrays… and possibly future built-in array types to come (such as
[uniform arrays](https://github.com/ocaml/RFCs/pull/37)).

By default, array comprehensions are of type `'a array`, unless the inferred
type is `_ floatarray` or `_ iarray`, in which case an array of that type is
constructed.
...
array. If an array comprehension contains exactly one clause, and it’s
a `for ... and ...` clause, then an array of exactly the right size can be
allocated up front (instead of having to grow the generated array dynamically,
as is done in other cases). We call this the _fixed-size array comprehension
optimization_ below. This cannot be done with nested `for`s, as the sizes of
iterators further to the right could depend on the values generated by those on
the left; indeed, this is one of the reasons we have `for ... and ...` instead
of just allowing the user to nest fors.

In the general case, the structure is: we allocate a `Dynarray.t`; each
iterator becomes a loop; `when` clauses become an `if` expression, same as with
lists; and in the body, generated values are appended to the dynarray.
This means that at the end, we have to convert that dynarray to the required
array type.
...
check for integer overflow, in which case we fail; a size greater than
`Sys.max_array_length` also results in a failure. We also check to see if any
of the iterators would be empty (have size 0), in which case we can shortcut
this whole process and return an empty array. Once we do that, though, we don’t
need to convert to a fixed-size array at the end.
...
The loop body is tighter as the array doesn't need to be reallocated, but
there’s more up-front size checking work to be done.
